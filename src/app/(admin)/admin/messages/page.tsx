...
export default function AdminMessagesPage() {
  ...
  const totalUnread = inbox?.reduce((sum, t) => sum + t.unreadCount, 0) ?? 0;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Admin"
        title="Messages"
        lead={`Direct messaging with teachers and branch members. ${totalUnread > 0 ? `${totalUnread} unread message${totalUnread > 1 ? "s" : ""}.` : ""}`}
        breadcrumbs={[
      ...
      <div className="flex gap-4 h-[calc(100vh-240px)] min-h-[500px]">
        {/* LEFT PANEL — Inbox */}
        <div className={`w-full md:w-[320px] flex flex-col gap-2 shrink-0 ${selectedUserId || composing ? "hidden md:flex" : "flex"}`}>
            ...
            <CardContent className="p-0">
              {!inbox ? (
                ...
                <div className="p-6 text-center">
                  ...
                  <p className="text-xs text-[var(--ink-muted)] mt-1">Click <strong>New Message</strong> to reach a teacher</p>
                </div>
              ) : (
                filteredInbox?.map((t) => (
                  <button
          ...
          {composing && !selectedUserId && (
            <Card className="flex-1 flex flex-col">
              <div className="p-4 border-b border-[var(--line)] flex items-center gap-3">
                ...
                <span className="font-semibold text-[var(--ink)]">New Message — Select teacher</span>
              </div>
                ...
                {filteredUsers?.map((u) => (
                  ...
                  >
                    ...
                    <a
                      ...
                      className="p-2 rounded-[var(--r-md)] text-[var(--union)] hover:bg-[var(--union-soft)] transition-colors shrink-0"
                      title={`Call ${u.phone}`}
                    >
          ...
          {!selectedUserId && !composing && (
            <Card className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-[var(--ink-muted)] mt-1">or click <strong>New Message</strong> to contact a teacher</p>
              </div>
            </Card>
          )}
        </div>
